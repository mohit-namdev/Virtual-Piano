...
for i in range(len(white_notes)):
    white_sounds[i] = mixer.Sound(f'PythonPiano-main\\assets\\notes\\{white_notes[i]}.wav')

for i in range(len(black_notes)):
    black_sounds[i] = mixer.Sound(f'PythonPiano-main\\assets\\notes\\{black_notes[i]}.wav')
...
# Recording class
class RecordedNote:
    def __init__(self, note, duration, color):
        self.note = note
...
def save_recorded_notes(filename, recorded_notes):
    print("Savinng")
    with open(filename, 'w') as file:
        for note in recorded_notes:
            color, i = note.split()
            file.write(f"{color} {i}\n")

def load_recorded_notes(filename):  
    recorded_notes = []
    with open(filename, 'r') as file:
        for line in file:
            note, duration, color = line.split()
            note_obj = RecordedNote(note, int(duration), color)
            recorded_notes.append(note_obj)
            note_obj.play()
...
# Main Loop
recording = False  # Set recording to False initially
run = True
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for i in range(len(black_keys)):
                if black_keys[i].collidepoint(event.pos):
                    note = RecordedNote(black_labels[i], pygame.time.get_ticks(), "black")
                    if recording:
                        recorded_notes.append(f"black {i}")
                    note.play(i)
                    black_key = True
            for i in range(len(white_keys)):
                if white_keys[i].collidepoint(event.pos) and not black_key:
                    note = RecordedNote(white_notes[i], pygame.time.get_ticks(), "white")
                    if recording:
                        recorded_notes.append(f"white {i}")
                    note.play(i)
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_q:  # Toggle recording when 'Q' is pressed
                recording = not recording
                if recording:
                # Check if the key pressed corresponds to a piano key
                    if event.unicode.upper() in left_dict:
                        recorded_notes.append(RecordedNote(left_dict[event.unicode.upper()], pygame.time.get_ticks()))
                    if event.unicode.upper() in right_dict:
                        recorded_notes.append(RecordedNote(right_dict[event.unicode.upper()], pygame.time.get_ticks()))
                else:
                    # Save recorded notes when recording is stopped
                    save_recorded_notes("recorded_notes.txt", recorded_notes)
            if event.key == pygame.K_a:  # Load recorded notes when 'A' is pressed
                recorded_notes = load_recorded_notes("recorded_notes.txt")
