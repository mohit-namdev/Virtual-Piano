...
while run:
    ...
    if keys[pygame.K_q]:  # Changed from pygame.K_r to pygame.K_q
        recording = not recording
        if recording:
            recorded_notes = []  # Clear recorded notes when starting a new recording

    # Record notes while recording is active
    if recording:
        # Check for mouse clicks and keyboard inputs to record notes
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                # Check if the mouse click is on a piano key
                for i in range(len(black_keys)):
                    if black_keys[i].collidepoint(event.pos):
                        # Append the recorded note to the recorded_notes list
