...
def save_recorded_notes(filename, recorded_notes):
    with open(filename, 'w') as file:
        for note in recorded_notes:
            file.write(f"{note.note} {note.duration}\n")
...
def load_recorded_notes(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            recorded_notes.append(RecordedNote(note, int(duration)))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for i in range(len(black_keys)):
                if black_keys[i].collidepoint(event.pos):
                    if recording:
                        recorded_notes.append(RecordedNote(black_labels[i], pygame.time.get_ticks()))
            ...
            for i in range(len(white_keys)):
                if white_keys[i].collidepoint(event.pos) and not black_key:
                    if recording:
                        recorded_notes.append(RecordedNote(white_notes[i], pygame.time.get_ticks()))
